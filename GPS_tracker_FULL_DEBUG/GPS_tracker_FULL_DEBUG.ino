...
void loop() {
  ...
  if (GPS.newNMEAreceived()) {
    ...
    if (GPS.fix) {
      ...
      myTracks.push_back(Point(GPS.longitude,GPS.latitude));
      ...
      case ST_INIT:
        if (intersection(myTracks[0],myTracks[myTracks.size() - 1], myWaypoints[0])){
          ...
          myLEDs[LED_LAP].set(YELLOW);
          myLEDs[LED_SPLT].set(YELLOW, 3000);
          next_state = ST_RACE;
          wp_last = 0;
          wp_next = 1;
      ...
      case ST_RACE:
        ...
        if (intersection(myTracks[0],myTracks[myTracks.size() - 1], myWaypoints[wp_next])){
          ...
          if (myWaypoints[wp_next].split_margin < 0) myLEDs[LED_SPLT].set(GREEN, 3000);
          else if (myWaypoints[wp_next].split_margin > 100) myLEDs[LED_SPLT].set(RED, 3000);
          else myLEDs[LED_SPLT].set(YELLOW, 3000);

          if (myWaypoints[wp_next].lap_margin < 0) myLEDs[LED_LAP].set(GREEN);
          else if (myWaypoints[wp_next].lap_margin > 100) myLEDs[LED_LAP].set(RED);
          else myLEDs[LED_LAP].set(YELLOW);
