    ...
    if (GPS.fix) {
      ...
      case ST_RACE:
        ...
        if (intersection(myTracks[0],myTracks[myTracks.size() - 1], myWaypoints[wp_next])){
          ...
          myTracks.clear();
          Serial.println(myLEDs[LED_LAP].endTime);
          int blinkrate = 500;
          if (myWaypoints[wp_next].speed_margin > 1) {
            if (myWaypoints[wp_next].speed_margin > 5) blinkrate = 100;
            else if (myWaypoints[wp_next].speed_margin > 4) blinkrate = 200;
            else if (myWaypoints[wp_next].speed_margin > 3) blinkrate = 300;
            else if (myWaypoints[wp_next].speed_margin > 2) blinkrate = 400;
            myLEDs[LED_LAP].set(OFF);
            myLEDs[LED_SPLT].set(OFF);
          ...
          else if (myWaypoints[wp_next].speed_margin < -1) {
            if (myWaypoints[wp_next].speed_margin < -5) blinkrate = 100;
            else if (myWaypoints[wp_next].speed_margin < -4) blinkrate = 200;
            else if (myWaypoints[wp_next].speed_margin < -3) blinkrate = 300;
            else if (myWaypoints[wp_next].speed_margin < -2) blinkrate = 400;   
